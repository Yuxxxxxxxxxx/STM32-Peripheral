 ...
 * @brief      串口设备初始化
 * 
 * @param[in]  uart_id    :    选择设备（0: UART1， 1: UART2）
 * 
 * @return     none
 * 
******************************************************************************/
void uart_device_init(uint8_t uart_id)
{
    if (uart_id == 0) // USART1
    {
		...
		uart_dma_init(s_uart_dev[uart_id].dmarx_buf, sizeof(s_uart1_dmarx_buf)); 
		s_uart_dev[uart_id].status = 0;
    }
#ifdef USE_USART2
    else if (uart_id == 1) // USART2
    {
        /* 配置串口2收发fifo */
		...
		// 底层初始化函数是针对 USART1 的，如果要使用 USART2 则要更改底层引脚号以及DMA通道
...
void uart_dmarx_half_done_isr(uint8_t uart_id)
{
	...
	if (uart_id == 0)
	{
		recv_total_size = s_uart_dev[uart_id].dmarx_buf_size - bsp_uart_get_dmarx_buf_remain_size();
	}
#ifdef USE_USART2
	else if (uart_id == 1)
	{
...
void uart_poll_dma_tx(uint8_t uart_id)
{
	...
	if (size != 0)
	{
		...
		if (uart_id == 0)
		{
			...
			bsp_uart_dmatx_config(s_uart_dev[uart_id].dmatx_buf, size);
		}
	#ifdef USE_USART2
		else if (uart_id == 1)
		{
